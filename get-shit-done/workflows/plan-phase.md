...
@/home/ollie/.config/opencode/get-shit-done/references/ui-brand.md
</required_reading>
...
```bash
INIT=$(node /home/ollie/.config/opencode/get-shit-done/bin/gsd-tools.cjs init plan-phase "$PHASE")
```
...
```bash
PHASE_INFO=$(node /home/ollie/.config/opencode/get-shit-done/bin/gsd-tools.cjs roadmap get-phase "${PHASE}")
```
...
```bash
PHASE_DESC=$(node /home/ollie/.config/opencode/get-shit-done/bin/gsd-tools.cjs roadmap get-phase "${PHASE}" | jq -r '.section')
PHASE_REQ_IDS=$(node /home/ollie/.config/opencode/get-shit-done/bin/gsd-tools.cjs roadmap get-phase "${PHASE}" | jq -r '.section // empty' | grep -i "Requirements:" | head -1 | sed 's/.*Requirements:\*\*\s*//' | sed 's/[\[\]]//g' | tr ',' '\n' | sed 's/^ *//;s/ *$//' | grep -v '^$' | tr '\n' ',' | sed 's/,$//')
```
...
```
Task(
  prompt="First, read /home/ollie/.config/opencode/agents/gsd-phase-researcher.md for your role and instructions.\n\n" + research_prompt,
...
**If found:**
1. Read validation template from `/home/ollie/.config/opencode/get-shit-done/templates/VALIDATION.md`
...
```bash
node /home/ollie/.config/opencode/get-shit-done/bin/gsd-tools.cjs commit-docs "docs(phase-${PHASE}): add validation strategy"
```
...
```
Task(
  prompt="First, read /home/ollie/.config/opencode/agents/gsd-planner.md for your role and instructions.\n\n" + filled_prompt,
  subagent_type="general",
  model="{planner_model}",
...
Task(
  prompt="
    ...
    <execution_context>
    @/home/ollie/.config/opencode/get-shit-done/workflows/execute-phase.md
    ...
    @/home/ollie/.config/opencode/get-shit-done/references/model-profile-resolution.md
    </execution_context>
