...
```bash
INIT=$(node /home/ollie/.config/opencode/get-shit-done/bin/gsd-tools.cjs init quick "$DESCRIPTION")
```
...
```bash
node /home/ollie/.config/opencode/get-shit-done/bin/gsd-tools.cjs commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${file_list}
```
