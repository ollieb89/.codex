...
```bash
INIT=$(node /home/ollie/.config/opencode/get-shit-done/bin/gsd-tools.cjs init phase-op "${after_phase}")
```
...
```bash
RESULT=$(node /home/ollie/.config/opencode/get-shit-done/bin/gsd-tools.cjs phase insert "${after_phase}" "${description}")
```
